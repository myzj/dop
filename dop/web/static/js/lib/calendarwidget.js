define('lib/calendarwidget', function() {
		var calendar = function (data) {
			var CalendarHandler = {
			 ...
			 $calendarItem = this.CreateCalendar(0, 0, 0, data.TodayDate);
			 $("#Container").append($calendarItem);
			if(data.callback != null && (typeof data.callback == "function")){
				data.callback();
			 }
		     ...
		     $(".currentItem>a").css("margin-left", ($(".item").width() - 25) / 2 + "px").css("padding-top", ($(".item").height() - 25) / 8 + "px");
			 $(".week>h3").css("width", $(".week").width() / 7 + "px");
			 $(".week>h3:last-child").css("width", $(".week").width() / 7-1 + "px");
			 //this.RunningTime();
			 },
			...
			if(data.callback != null && (typeof data.callback == "function")){
			 ...
			 CreateCalendar: function (y, m, d, realDate) {
			 $dayItem = $("<div class=\"dayItem\"></div>");
			 //获取当前月份的天数
			 if (realDate != null) {
			 var nowDate = new Date(realDate);
			 } else {
			 ...
			 $("#title").html(nowYear + " - " + nowMonth + " - " + nowDay);
			...
			var _newDateArr = (data.TodayDate).split("/");
			for (var i = 0; i < nowDaysNub; i++) {
				  if (i == (nowDay - 1)) {
					  if(isInArray(data.MemberSignRecords, _newDateArr[0] +"/"+ _newDateArr[1] +"/"+ (i + 1))[0] != null ){
						  if(isInArray(data.MemberSignRecords, _newDateArr[0] +"/"+ _newDateArr[1] +"/"+ (i + 1))[0].res == -1){
							  $dayItem.append("<div class=\"item currentItem\"><a data-day="+ (i + 1) +">" + (i + 1) + "</a></div>");
						  }else if(isInArray(data.MemberSignRecords, _newDateArr[0] +"/"+ _newDateArr[1] +"/"+ (i + 1))[0].res == 1){
								$dayItem.append("<div class=\"item currentItem currentItem1\"><a data-day="+ (i + 1) +">" + (i + 1) + "</a></div>");
						  }else if(isInArray(data.MemberSignRecords, _newDateArr[0] +"/"+ _newDateArr[1] +"/"+ (i + 1))[0].res  == 2){
								$dayItem.append("<div class=\"item currentItem currentItem2\"><a data-day="+ (i + 1) +">" + (i + 1) + "</a></div>");
						  }else if(isInArray(data.MemberSignRecords, _newDateArr[0] +"/"+ _newDateArr[1] +"/"+ (i + 1))[0].res == 3){
								$dayItem.append("<div class=\"item currentItem currentItem3\"><a data-day="+ (i + 1) +">" + (i + 1) + "</a></div>");
						  }
					  }else{
						  $dayItem.append("<div class=\"item currentItem\"><a data-day="+ (i + 1) +">" + (i + 1) + "</a></div>");
					  }
					  ...
					  if(isInArray(data.MemberSignRecords, _newDateArr[0] +"/"+ _newDateArr[1] +"/"+ (i + 1)).length > 0){
						  if(isInArray(data.MemberSignRecords, _newDateArr[0] +"/"+ _newDateArr[1] +"/"+ (i + 1))[0] != null ){
							if(isInArray(data.MemberSignRecords, _newDateArr[0] +"/"+ _newDateArr[1] +"/"+ (i + 1))[0].res == -1){
								  $dayItem.append("<div class=\"item currentItem\"><a data-day="+ (i + 1) +">" + (i + 1) + "</a></div>");
							  }else if(isInArray(data.MemberSignRecords, _newDateArr[0] +"/"+ _newDateArr[1] +"/"+ (i + 1))[0].res == 1){
								  $dayItem.append("<div class=\"item currentItem item1\"><a data-day="+ (i + 1) +">" + (i + 1) + "</a></div>");
							  }else if(isInArray(data.MemberSignRecords, _newDateArr[0] +"/"+ _newDateArr[1] +"/"+ (i + 1))[0].res == 2){
								  $dayItem.append("<div class=\"item currentItem item2\"><a data-day="+ (i + 1) +">" + (i + 1) + "</a></div>");
							  }else if(isInArray(data.MemberSignRecords, _newDateArr[0] +"/"+ _newDateArr[1] +"/"+ (i + 1))[0].res == 3){
								  $dayItem.append("<div class=\"item currentItem item3\"><a data-day="+ (i + 1) +">" + (i + 1) + "</a></div>");
							  }else{
								$dayItem.append("<div class=\"item currentItem\"><a data-day="+ (i + 1) +">" + (i + 1) + "</a></div>");
			...
			CalendarHandler.initialize(data);
		};
		//判断数组是否存在某个值
	function isInArray(obj, val){
			var jsonArr = [];
            var res = -1;
            $.each(obj, function(i, item){
                if(item.SignDate == val){
                    var json = {};
                    if(item.SpecialFlag != null){ //发奖日子，1是优惠券，2 是小礼物
                        if(item.SpecialFlag == 1){
                            res = 1;
                        }else if(item.SpecialFlag == 2){
                            res = 2;
                        }
                    }else{
						 res = 3;
